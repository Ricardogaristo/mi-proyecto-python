...
BACKUP_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bbdd")

def hacer_backup(manual=False):
    """Exporta toda la BD a un archivo JSON en la carpeta bbdd/."""
    ...
    conn = get_connection()
    backup_data = {"timestamp": timestamp, "tipo": tipo, "tablas": {}}

    try:
        with conn.cursor() as cursor:
            for tabla in ["usuarios", "tareas", "subtareas", "alumnos",
                          "historial_snapshots", "historial_automatico", "alarmas_completadas"]:
                try:
                    cursor.execute(f"SELECT * FROM {tabla}")
                    rows = cursor.fetchall()
                    # Convertir fechas/date a string
                    clean = []
                    for row in rows:
                        clean_row = {}
                    ...
                    backup_data["tablas"][tabla] = clean
                except Exception:
    ...
    # Conservar solo los últimos 10 backups automáticos
    if not manual:
        autos = sorted([
            x for x in os.listdir(BACKUP_DIR) if x.startswith("backup_auto_")
        ])
        while len(autos) > 10:
...
def _scheduler_backup():
    """Hilo en segundo plano: hace backup automático cada 24 horas."""
    while True:
        time.sleep(24 * 60 * 60)  # 24 horas
        try:
            nombre = hacer_backup(manual=False)
            print(f"✅ Backup automático: {nombre}")
...
def backup_manual():
    try:
        nombre = hacer_backup(manual=True)
...
def backup_lista():
    os.makedirs(BACKUP_DIR, exist_ok=True)
    archivos = sorted([
        x for x in os.listdir(BACKUP_DIR) if x.endswith(".json")
    ], reverse=True)
...
def backup_restaurar():
    ...
    # Seguridad: solo archivos .json dentro de bbdd/
    if not archivo.endswith(".json") or "/" in archivo or "\\" in archivo:
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"Error leyendo backup: {e}"}), 500

    tablas = backup.get("tablas", {})
    conn = get_connection()

    # Orden de restauración respetando foreign keys
    orden = ["usuarios", "tareas", "subtareas", "alumnos",
             "historial_snapshots", "historial_automatico", "alarmas_completadas"]
...
if __name__ == "__main__":
    ...
    # Backup automático cada 24h en hilo separado
    t = threading.Thread(target=_scheduler_backup, daemon=True)
    t.start()
